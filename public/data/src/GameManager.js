class EJS_GameManager {
  ...
  async getSaveFileFromServer(loadedROM) {
    ...
    const apiUrl = serveraddres + ":" + serverport;
    const token = localStorage.getItem("token");

    const loadedgame = loadedROM;
    const buildRequest =
      apiUrl + "/api/savemanagement/getsavefile?gamefile=" + loadedgame;
    ...
    fetch(buildRequest, {
      ...
      .then((response) => {
        ...
        if (response.status === 404) {
          throw new Error("Savefile doesn't exist");
        }
        if (!response.ok) {
      ...
      //Building and writing binary file
      .then((data) => {
        console.log("Data fetched from API :", data);

        const saveFilePath = EJS_emulator.gameManager.getSaveFilePath();
        console.log("File save location :", saveFilePath);

        // build binary file
        const fileName = data[0].file_name;
        const binaryData = new Uint8Array(data[0].data);

        //Writing file in browser IDB
        try {
          EJS_emulator.Module.FS.writeFile(`${saveFilePath}`, binaryData);
          console.log(`file ${fileName} regenerated at : ${saveFilePath}`);
        } catch (err) {
          console.error("Failed writing file:", err);
        }
      })
      // Loading savefile in emulator memory
      .then((loadingsave) => {
        console.log("Loading save in emu");
        this.loadSaveFiles();
      })
      .catch((err) => {
  ...
  async setSaveFileToServer() {
    ...
    try {
      // Reading save file
      const fileData = EJS_emulator.Module.FS.readFile(filePath);
      const gameName = EJS_emulator.fileName;
      const token = localStorage.getItem("token");
      let apiUrl;
      let jsonData;

      // conversion to octet
      const binaryArray = Array.from(fileData);

      this.checkSaveFileExist(gameName).then((saveid) => {
        if (!saveid) {
          console.log("NO SAVE FILE FOUND CREATING A NEW ONE");

          jsonData = {
            fileName: filePath.substr(
              filePath.length - EJS_emulator.fileName.length
            ),
            size: fileData.length,
            data: binaryArray,
            //game: EJS_emulator.fileName
            game: gameName,
          };

          apiUrl =
            serveraddres + ":" + serverport + "/api/savemanagement/setsavefile"; // URL de l'API
        } else {
          console.log("UPDATING SAVE FILE");
          ...
          apiUrl =
            ...
            ":" +
            serverport +
            "/api/savemanagement/updatesavefile"; // URL de l'API
        }

        // Sending JSON through API
        ...
        fetch(apiUrl, {
          ...
          body: JSON.stringify(jsonData), // converting JSON to string (PostgreSQL compatibility)
        })
          .then((response) => {
            if (!response.ok) {
  ...
  checkSaveFileExist(gameFileName) {
    ...
    const apiUrl =
      ...
      ":" +
      serverport +
      "/api/savemanagement/savefileexists?fileName=" +
      gameFileName;
    ...
    return fetch(apiUrl, {
      ...
      .then((response) => {
        if (response.status === 404) {
          console.log("Returned 404 from api");
          return null;
        }
        return response.json();
      })
